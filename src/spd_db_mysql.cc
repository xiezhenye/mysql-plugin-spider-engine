...
int spider_db_mysql_row::store_to_field(
  Field *field,
  const CHARSET_INFO *access_charset
) {
...
int spider_db_mysql::append_lock_tables(
  ...
  const char *db_name;
  uint db_name_length;
  const CHARSET_INFO *db_name_charset;
  const char *table_name;
  uint table_name_length;
  const CHARSET_INFO *table_name_charset;
...
int spider_db_mysql_util::append_name_with_charset(
  ...
  const char *name,
  uint name_length,
  const CHARSET_INFO *name_charset
) {
  ...
  } else if (item_func->functype() == Item_func::UNKNOWN_FUNC)
  {
    if (
      ...
      if (str)
      {
        ...
        const CHARSET_INFO *conv_charset = item_func_conv_charset->conv_charset;
